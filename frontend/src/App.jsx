...
function App() {
    ...
    useEffect(() => {
        const verifyCookie = async () => {
            try {
                const { data } = await axios.get("http://localhost:8080/auth/verify-cookie", { withCredentials: true });
    ...
    return (
        <Routes>
            ...
            {role === "teacher" ? (
                ...
                <>
                    <Route path="/dashboard" element={<StudentLandingPage username={username} onLogout={Logout} />} >
                        <Route index element={<StudentHome username={username} onLogout={Logout} />} />
